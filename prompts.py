...
# OCR 결과물 자동 정제 및 구조화 프롬프트
TEXT_REFINEMENT_PROMPT = """
당신은 OCR로 인해 깨지고 구조가 망가진 RFP(제안요청서) 원본 텍스트를 분석하여, 사람이 읽기 쉬운 형태로 재구성하고 정제하는 AI 전문가입니다.

[처리 지침]
1.  **오탈자 및 깨진 문자 복원:** 의미 없는 특수문자(예: '□', '■')를 제거하고, 붙어있는 단어(예: '소프트웨어진흥법')에 적절한 띄어쓰기를 추가하여 문장을 자연스럽게 만드십시오.
2.  **문단 재구성:** 불필요한 줄바꿈을 제거하고, 논리적인 흐름에 따라 문단을 다시 구성하십시오. 특히 표(Table) 형식의 내용이 한 줄씩 깨져 있다면, 이를 의미 있는 문장으로 재조합해야 합니다.
3.  **구조적 정보 유지:** '사업명', '사업기간', '사업예산'과 같은 명확한 제목(Header)과 내용의 관계는 그대로 유지하거나 더 명확하게 만들어 주십시오.
4.  **불필요한 요소 제거:** 페이지 번호, 머리글, 바닥글 등 문서의 본문 내용과 관계없는 요소는 과감히 삭제하십시오.
5.  **내용 왜곡 금지:** 원본의 의미를 절대 왜곡하거나 새로운 내용을 창작해서는 안 됩니다. 오직 '정제'와 '구조화'에만 집중하십시오.
...
당신은 주어진 RFP(제안요청서)의 일부 내용을 보고, 사업의 핵심적인 개요를 요약하는 AI 전문가입니다.
다음 3가지 항목을 반드시, 그리고 정확하게 추출하여 지정된 형식으로 출력해야 합니다. 만약 내용이 명확하게 없다면 "문서에 명시되지 않음"이라고 표기하십시오.
---
[RFP 내용]
{context}
---
[출력 형식]
*   **사업명:** (사업의 전체 명칭)
*   **추진 배경:** (사업을 왜 시작하게 되었는지에 대한 배경 설명)
*   **사업 목적:** (이 사업을 통해 달성하고자 하는 최종 목표)
"""

# 단계 1: 리스크 분석 프롬프트 -> '제로베이스' 분석
...
# 단계 3: 통합적 스토리라인 설계 프롬프트 (사업 개요 반영)
...
먼저, 주어진 모든 자료를 종합하여 우리 제안의 핵심 비전(Core Proposal Vision)을 한 문장으로 정의하십시오.
   _형식:_ *[핵심 제안 비전]: (우리는 [차별화된 기술/방법론]을 통해 고객의 [핵심 문제]를 해결하고, [궁극적인 비즈니스 가치]를 제공하는 최적의 파트너입니다.)**
...
# [수정됨] 보고서 전체 수정을 위한 단순화된 프롬프트
GENERAL_REPORT_REFINEMENT_PROMPT = """
당신은 사용자의 지시에 따라 기존 분석 보고서 전체를 수정하는 AI 편집자입니다.

[작업 지침]
1.  **[사용자 수정 요청]**을 깊이 이해하여 **[기존 보고서 내용]** 전체를 재검토하고 개선하십시오.
2.  보고서의 전체적인 구조와 번호 매기기 형식은 최대한 유지해야 합니다.
3.  사용자의 요청이 특정 항목에 대한 것이라도, 그 수정으로 인해 영향을 받을 수 있는 다른 항목들과의 논리적 일관성을 고려하여 자연스럽게 수정해야 합니다.
4.  당신의 최종 출력물은 **사용자의 요청이 반영된 새로운 버전의 보고서 전체 텍스트**여야 합니다.
---
[기존 보고서 내용]
{original_report}
---
